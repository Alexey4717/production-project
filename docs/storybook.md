...
В Storybook настройки могут быть сделаны в нескольких местах:
main.ts — здесь настраиваются общие параметры Storybook, такие как stories, addons, и webpackFinal. Это главный конфигурационный файл, который определяет, как Storybook будет работать и какие дополнения подключать.
preview.ts — этот файл отвечает за настройки, которые касаются того, как компоненты отображаются в Storybook. Здесь могут быть добавлены декораторы, параметры или глобальные стили.
...
UPD обновил сторибук до 8 версии.
Для моков поменял пакет на msw.
Нужно было создать сервис-воркер командой npx msw init public для корректной инициализации msw.
