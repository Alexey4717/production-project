...
Инициализация проекта для применения дефолтных настроек
...
Так же создали папку src, где хранится исходный код приложения.
Для начала в src создали index.js и test.js для проверки работы импортов.
В test.js сделали import из index.js.
Запустить скрипт не получилось, т.к. нужны настройки для работы таких импортов.
Тут на помощь приходит вебпак.
Это сборщик, который позволяет гибко сконфигурировать сборку приложения.
Т.е. в исходниках может быть много разных файлов, разных расширений,
а по итогу вебпак вернет статику (hmtl, css, js, файлы изображений), которую можно загрузить на облачный хостинг и приложение будет доступно в интернете.
Вебпак так же позволяет оптимизировать код (сжать, минимизировать).

Инициализация вебпака (Для работы с webpack через командную строку):

```bash
npm install --save-dev webpack webpack-cli
```

Как дев зависимости, потому, что учавствуют они только на этапе разработки.
После того, как проект собрали, эти зависимости уже не нужны.
Создали webpack.config.ts (конфигурация webpack сборки).
Позже вынесем часть конфига в config/build/buildWebpackConfig.ts,
а в самом webpack.config.ts оставим только определение path и других переменных

Изначально в нем используем module.exports = {} (экспорты для NodeJS).
...
Хардкодить пути не очень хорошая практика, поэтому их прописываем через path.resolve.
...
Т.к. на разных ОС пути работаю по-разному (dirname - папка в которой мы находимся сейчас - где находится текущий файл)
Сборка будет идти в папку build (иногда называют dist).

Для запуска сборки достаточно написать команду webpack.
Теперь будут доступны import/export в исходниках (webpack преобразовал их в импорты на NodeJS в build папке).
Так же в конфиге нужно указывать mode (development или production).
- production - для публикации сборки
- development - на этапе разработки
Если вебпак видит mode - production он автоматически оптимизирует код (минификация)

В браузере есть механизм кеширования файлов чтобы быстро отдавать их из кеша, не загружая каждый раз из сервера.
Нужно в конфиге указывать output.filename: '[name].[contenthash].js'
Это шаблоны для динамических названий файлов, чтоб браузер использовал всегда актуальные файлы.
name - название entrypoint, по дефолту main
contenthash - для хеширования, чтоб отдавался новый контент в браузере после обновления сборки
Будут генериться уникальные идентификаторы, типа main.0a8e7fd5e3294937b2fb.js (и проблема с кешированием решена).
Можно указать несколько энтрипоинтов (не только main), например:
...
После билда появится помимо main.js еще и RANDOM.js

Создали public/index.html, для имплементации туда react приложения.
У вебпака есть много плагинов для разных операций.
Установили html-webpack-plugin (как devDep). Чтоб сборщик мог собирать html файлы
Для применения в конфиге указываем свойство plugins (все плагины потом будем выносить в функцию config/build/buildPlugins.ts).
Потом после команды билда будет файл index.html
Там в body не будет div.root (нужно указать template в плагине), но будет script со ссылкой на текущий main.0a8e7fd5e3294937b2fb.js. 
Удобно, что не нужно руками подключать файлы скриптов из исходников.

Нужно за сборкой следить и понимать сколько % уже произошло (для этого progressPlugin).

Для работы с typescript нужно установить в devDeps пакеты typescript, ts-loader
Далее создать файл tsconfig.json и в исходниках js файлы поменять на ts.
В конфиге вебпака указали поле resolve (настройки вынесем в config/build/buildResolvers.ts).
Так же указали module.rules (настройки вынесем в config/build/buildLoaders.ts). Там будет обработка всех файлов, кроме js.

В лоадерах:
- test - регуларка с расширением файла, по которому надо проходиться
В regex101.com можно проверить регулярку на то, ловит ли regular expression строки test string
- use - указываем лоадер, который нужно использовать
- exclude - исключение для обработки (node_modules)

Так же нужно в tsconfig указать ts-node.compilerOptions.module

Теперь после билда будут js файлы, собранные из ts файлов исходников.
Обычно если сборка падает с ошибкой, нужно смотреть чего не хватает (лоадера, плагина).

Добавим в tsconfig поле allowSyntheticDefaultImports, чтоб не импортировать import * as Package, а сразу import Package from (даже если у пакета нет дефолтного импорта).

## 2. Декомпозиция конфига. Опции конфигурации.

Конфиг начинает быстро разрастаться, поэтому необходимо его декомпозировать, чтоб привезте к читабельному виду.
В корне проекта создали папку config, где будет храниться вся конфигурация приложения.
Там будут подпапки, в которых будем понфигурировать разные среды (например jest, storybook, eslint (если понадобится писать плагины)).
В config/build описываются сценарии конфигурации вебпака.
Для каждого вебпак свойства создается отдельный файл (buildDevServer, buildLoaders, buildPlugins, buildResolvers, buildWebpackConfig).
В оснвонм конфиге будем вызывать эти функции для присвоения результата.
В эти функции добавляем нужную типизацию, чтоб если что-то лишнее туда добавлялось на этапе разработки ругался ts.
Не забываем менять пути в path.resolve (но лучше вынести из глобально и передавать в options извне, что сделаем далее).

- buildPlugins - возвращает список плагинов
- buildLoaders - возвращает список лоадеров (порядок лоадеров в массиве имеет значение, нужно быть внимательным, выносить лоадеры в переменные чтоб четче видеть последовательность)
- buildResolvers - возвращает объект с настройками для resolve
- buildWebpackConfig - функция, которая возвращает всю конфигурацию webpack

Чтобы задавать пути еще до начала этапа сборки (конфигурировать конфиг) сделаем механизм для определения опций извне,
например порт, пути (paths), адрес api с которым будем работать, режим (development/production), locales и т.п.
Этим всем хорошо работать извне. Описали типы для опций сборки в config/build/types.
Описали пути (до энтри понта, до html, до src, все которые будут использоваться в сборке). Это глобальные внешние настройки.

Технически, описываем (создаем) эти опции в корневом файле webpack.config.ts
Передаем их в config/build/buildWebpackConfig.ts и возвращаем из конфига.
Эти options также будут передаваться дальше (в buildLoaders, buildPlugins, buildResolvers) из buildWebpackConfig и переиспользоваться там.

Для удобства сделали переменную isDev = mode === 'development'

## 3. Webpack-dev-server. Переменные окружения (env)

Установили пакет webpack-dev-server (и типы для него) в devDeps.
Если внести изменение в исходники, то чтобы их уведеть в браузере, необходимо заново сделать сборку. Это не удобно.
Dev-server нужен, чтобы автоматически перезапускать сборку после изменения исходников и отдавать обновлённые файлы.
Добавили для isDev devtool: inline-source-map (позже заменили на eval-cheap-module-source-map) для отслеживания ошибки по stacktrace,
где она произошла (т.к. из нескольких файлов исходников сборщик слепляет в один bundle.js).
Сделали watch режим дев-сервера

Создали файл config/build/buildDevServer.ts с настройками дев-сервера.
Применили настройки в корневом конфиге для поля devServer (для isDev).

В webstorm можно исправить работу с относительными импортами settings -> code style -> typescript -> imports (use Path Relative to tsconfig).

в package.json скрипт для разработки через дев сервер "start": "webpack serve",

Для отделения дев сборки от прод сборки, нужно разделять это на уровне скриптов и переменных окружений.
Они активно используются на фронте и бэке, при сборке.
В скрипте указывается флаг --env и через пробел сколь угодно много определений переменных
"webpack serve --env port=3000"

Сделали 2 скрипта для сборок на разных окружениях с передачей разных переменных окружений
"build:prod": "webpack --env mode=production",
"build:dev": "webpack --env mode=development",

Для получения переменных окружений в сборке, в корневом файле webpack.config.ts выполнили конфиг в виде функции,
которая в аргументах получает env объект со всеми переменными.
Сделали тип BuildEnv для этих переменных окружения.
В webpack.config.ts получаются эти значения переменных (либо дефолтные) и передаются дальше по декомпозированным файлам конфига.
Т.к. переменные окружения передают в скриптах разные mode, то вебпак по-разному собирает сборку (для дев и прод).
Скрипт прод можно будет использовать на удалённом окружении для сборки и разворачивания проекта.

По-хорошему надо тестировать прод сборку, чтоб там не было лишних комментов и прочего кода, чтоб он был минимизирован.
Для этого в разные поля конфига добавлять переменную isDev (отключать разные свойства для prod сборки, например source-map, dev-server).

## 4. Подключаем React и настраиваем css в webpack.

Установим пакеты react react-dom (и типы для них). Типы в devDeps, основные пакеты в deps.
Для работы с jsx нужно подключить лоадеры.
Т.к. лоадеров будет не мало, то вынесли их определение в config/build/loaders. В каждом файле будут определяться лоадеры.
Можно использовать typescriptLoader для jsx. Если использовать нативный js (без ts), то достаточно только babelLoader.

Создали src/index.tsx файл с render 1-го компонента (поменяли путь до энтрипоинта - index.tsx).
Добавили настройку в tsconfig "jsx": "react-jsx", чтобы не импортить React from react и читался jsx

Создали 1-й компонент Counter для тестирования.
Большинство компонентов нужно будет экспортировать через именованный экспорт, а не дефолтный.

Для работы с css нужно также добавить лоадер в конфиг вебпака (config/build/loaders/buildCssLoader.ts).
Установили пакеты sass sass-loader style-loader scc-loader в devDeps
...
Если в файле с типами и интерфейсами находится enum, то их нужно переносить в consts (допустимый сегмент в FSD),
...
Обязательное условие, чтоб index.html был в корне.
...
Сделали redesigned input, селекторы валюты и страны. Карточку профиля.

## 139. Градиент на иконке, недочеты.
Иконка логотипа была засвеченная, т.к. под градиентом. Исправили это.
Исправили sideBarItem и скелетоны.

## 140. Редизайн ленты статей.
TODO вернуть потом виртуализацию, только с другой либой.
Изменили отображения списка статей по новому дизайну.

## 141. Редизайн плиточного отображения статей.
Сломалась вёрстка отображения списка статей в виде плитки, починили.

## 142. Переключение нового и старого дизайна. Страница настроек. Обновление фичей.
Пофиксили Skeleton в NotificationList.
...
## 143. Рефакторинг портала, модалки, шторки.
Modal и Drawer перенесли в redesigned. У них сделали портал в app а не body, чтобы глобал стили применялись к ним тоже.
А из index.scss их удалили для них.
...
TODO убрать max (вместо него fullWidth), оставить fullWidth и fullHeight у Card.

## 145. Редизайн ArticleDetails.
Тут редизайнили страницу статьи.

## 146. Редизайн рейтинга, комментариев.
StarRating сделали с ToggleFeature внутри Deprecated компонента. Чтоб не дублировать код, который отличается только в иконках.

## 147. Редизайн модалок, устранение мелких недочетов.
TODO сделать стили для Button filled

## 148. ForceUpdateProvider костыль. Обновляем интерфейс.
...
## 149. Редизайн скролла. Фикс скелетона.
Сделали в ThemeProvider изменение класса для body
     document.body.className = theme
Чтоб изменялся цвет скролбара в зависимости от темы.

## 150. AppLoaderLayout. Fallback для темы и выбранно.
Сейчас при инициализации приложения есть лоадер.
Хочется сделать лэйаут на основе скелетонов, который будет имитировать загрузку интерфейса.
Создали shared/layouts/AppLoaderLayout
В основе него лежит MainLayout, наполненный контентом из скелетонов.
Использовали в App. Для старой темы обычный лоадер, для новой - AppLoaderLayout.
Сделали сохранение последней выбранной темы в localStorage для fallback.
Чтоб не было ситуации, когда к примеру пользователь ночью сидит на тёмной теме, перезагружает страницу, у него белый экран и его слепит.

## 151. Toolbar как часть Layout. ScrollToolbar. useCurrentRoute.
Сделаем механизм, который гибко, в зависимости от страницы, будет подменять содержимое rightbar`а.
Т.е. где-то нужна кнопка скрола к топу, где-то ничего не нужно, где-то вместо кнопки нужно отобразить ещё что-нибудь.
Реализовали features/scrollToTopButton - кнопка скрола к топу.
Сделали фичей просто кнопку, которая скролит наверх. Она не знает, где будет находиться, просто фича, которая выполняет определённое действие.
А позиционировать эту фичу уже будем в виджете, в частности в widgets/ScrollToolbar.
Такие тулбары могут быть разные (напрмаер PaymentToolbar для оплаты и т.д.) и в зависимости от страницы можно эти тулбары модульно подменять.
В app/lib/useAppToolbar сделана проверка, на какой странице показывать scrollToolbar.
...
При запуске сторибука, во всех компонентах ошибка редакса. В ThemeDecorator мы оборачиваем все сторикейсы в конфиге.
Это из-за того что в ThemeProvider добавили useJsonSettings и там напрямую обращение к редаксу.
Это явное нарушение зон ответственности. В ThemeProvider (независимый кусок логики, который отвечает за темы), внедрили бизнес логику получения настроек пользователя.
Сделаем это передачу в пропсах извне.
